...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  modifier checkValue(uint _upc) {
    ...
    address payable consumerID = address(uint160(items[_upc].consumerID));
  ...
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, "Item not ForSale");
    _;
  }
  ...
  function kill() public {
    if (msg.sender == owner()) {
      address payable origOwner = address(uint160(owner()));
  ...
  function buyItem(uint _upc) public payable
    ...
    address payable originFarmerID = address(uint160(items[_upc].originFarmerID));
    originFarmerID.transfer(items[_upc].productPrice);
    // emit the appropriate event
    emit Sold(_upc);
  }
